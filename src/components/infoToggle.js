...
const InfoToggle = () => {
  ...
  return (
    <div>
      <button onClick={toggleInfo} style={{ background: 'none', border: 'none', cursor: 'pointer'}}>
      ...
      {isVisible && (
        <div style={{ border: '1px solid #ccc', paddingRight: '5px', marginTop: '5px', fontStyle: "italic", fontSize: "15px", color: "red"}}>
          <ul>
            ...
            <li>Click<button onClick={toggleInfo} style={{ background: 'none', border: 'none', cursor: 'pointer' }}>
        <FontAwesomeIcon icon={faInfoCircle} size="lg" />
      </button> to disapear</li>
          </ul>
        </div>
      )}
    </div>
